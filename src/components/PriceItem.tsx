...
type PriceItemProps = { title: string, infoItems: string[] };

export const PriceItem = ({ title, infoItems }: PriceItemProps) => {
    const { breakpoints } = useTheme();
    const isSmallScreen = useMediaQuery(breakpoints.down(1000));

    return (
        <Stack border={2} borderRadius={15} borderColor='#000' height='fit-content' pb={2}>
